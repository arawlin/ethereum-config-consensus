...
../lighthouse/target/maxperf/lighthouse bn --network mainnet --disable-deposit-contract-sync --datadir $DIR_DATA --allow-insecure-genesis-sync --http --execution-endpoint=http://localhost:8551 --execution-jwt ../jwt.hex --logfile $DIR_DATA/logs/lighthouse.log --logfile-debug-level info > /dev/null 2>&1 &
